...
# Read in the data
students <- read_csv("C:/Users/yasas/OneDrive/Desktop/sandeep R/oulad-students.csv")

# Mutate variables
students <- students %>%
  mutate(result_group = ifelse(final_result %in% c("Distinction", "Pass"), "Good", "Poor")) %>%
  mutate(result_group = as.factor(result_group))

students <- students %>%
  mutate(age_group = ifelse(age_band == "0-35" | age_band == "35-55", "Young", "Old")) %>%
...
# Create a recipe
my_rec <- recipe(result_group ~ age_group + imd_band, data = data_train)
...
final_fit %>%
  collect_predictions() %>%
  select(.pred_class, result_group) %>%
  mutate(correct = .pred_class == result_group) %>%
  tabyl(correct)
